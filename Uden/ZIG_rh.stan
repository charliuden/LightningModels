...
transformed parameters {
  vector[N] alpha = exp(a_alpha + b_alpha * RH);  // Alpha must be positive
  vector[N] beta = exp(a_beta + b_beta * RH);    // Beta must be positive
  vector[N] pi = inv_logit(a_pi + b_pi * RH);    // Pi is constrained between 0 and 1
}
