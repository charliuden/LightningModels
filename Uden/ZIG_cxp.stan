...
transformed parameters {
  vector[N] alpha = exp(a_alpha + b_alpha * CXP);  // Alpha must be positive
  vector[N] beta = exp(a_beta + b_beta * CXP);    // Beta must be positive
  vector[N] pi = inv_logit(a_pi + b_pi * CXP);    // Pi is constrained between 0 and 1
}
