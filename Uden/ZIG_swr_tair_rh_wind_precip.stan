...
transformed parameters {
  vector[N] alpha = exp(a_alpha + b_alpha * SWR + c_alpha * T + d_alpha * RH + e_alpha * W + f_alpha * P);  // Alpha must be positive
  vector[N] beta = exp(a_beta + b_beta * SWR + c_beta * T + d_beta * RH + e_beta * W + f_beta * P);    // Beta must be positive
  vector[N] pi = inv_logit(a_pi + b_pi * SWR + c_pi * T + d_pi * RH + e_pi * W + f_pi * P);    // Pi is constrained between 0 and 1
}
