...
parameters {
  ...
  real b_pi;               // Slope for pi with SWR
}

// Transformed parameters to calculate the actual alpha, beta, and pi for each observation
transformed parameters {
  vector[N] alpha = exp(a_alpha + b_alpha * SWR);  // Alpha must be positive
  vector[N] beta = exp(a_beta + b_beta * SWR);    // Beta must be positive
  vector[N] pi = inv_logit(a_pi + b_pi * SWR);    // Pi is constrained between 0 and 1
}
